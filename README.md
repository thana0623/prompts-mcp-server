...
这会在 `~/.claude/CLAUDE.md` 中注册 `pmcp` 为已知全局工具。
之后在任何项目中输入 `pmcp start`，Claude Code 将直接执行，不再探索项目。

> 如果以后想移除：`pmcp unregister`
...
**简单说：装一次，`pmcp register` 一次，每个项目 `pmcp start` 一次，之后正常写代码就行。日志、上下文、角色切换都是自动的。**
